...
int scan_person(FILE* file, person* p) {
    if (fscanf(file, "%s %s %d %d %d %d\n", p->last_name, p->first_name, &p->proz, &p->gtx, &p->ssd, &p->hdd) == EOF) {
        return 0;
    }
    return 1;
}
...
int scan_person_from_stdin(person* p) {
    if (scanf("%s %s %d %d %d %d", p->last_name, p->first_name, &p->proz, &p->gtx, &p->ssd, &p->hdd) == EOF) {
        return 0;
    }
    return 1;
}
...
int main(int argc, char* argv[]) {
    ...
    int choice;
    if (argc < 1) {
        printf("Database file was not specified");
        exit(1);
    }
    ...
    while ((choice = enterChoice()) != 6) {
        switch (choice) {
            ...
            case 2:
                base_bin = fopen("out.bin", "rb");
                while (scan_person(base_bin,cur_person)) {
                    if (  (cur_person->gtx == 0 || cur_person->proz == 0 || (cur_person->hdd == 0 && cur_person->ssd == 0))) {
                        print_person(cur_person);
                    }
                }
                fclose(base_bin);
